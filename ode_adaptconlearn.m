function Xdot=ode_adaptconlearn(t,X)
global K1 K2 alpha gamma tprevious qdotprevious Y1Y1_sum Y1T1_sum flagcon prevcheck mineigvec tvec
...
%Adaptive Update Law
if flagcon==1
 thetahatdot=gamma*Y2'*r + K2*gamma*(Y1T1_sum - Y1Y1_sum*thetahat);  
else
...
%Using the time history of states for the concurrent learning part of the adaptive update law
if ~isempty(tprevious)
    if (t-tprevious)>10^-10
      if (t-prevcheck)>0.2
       qdoubledot=(qdot-qdotprevious)/(t-tprevious);
       q1doubledot= qdoubledot(1); q2doubledot= qdoubledot(2);
       Y1=[q1doubledot, q2doubledot, 2*q1doubledot*cos(q2) + q2doubledot*cos(q2) - q1dot*q2dot*sin(q2) - q2dot*sin(q2)*(q1dot + q2dot), q1dot, 0 ; 0, q1doubledot + q2doubledot, sin(q2)*q1dot^2 + q1doubledot*cos(q2), 0, q2dot];
        if isempty(Y1Y1_sum)
           Y1Y1_sum= Y1'*Y1;
           Y1T1_sum= Y1'*tau;
           tvec= t;
           mineigvec=min(eig(Y1Y1_sum));
        else
          lambda=0.5;
            if min(eig(Y1Y1_sum))> lambda
               flagcon=1;
            end
             if min(eig(Y1Y1_sum+ Y1'*Y1)) > min(eig(Y1Y1_sum))
                Y1Y1_sum= Y1Y1_sum + Y1'*Y1;
                Y1T1_sum= Y1T1_sum + Y1'*tau;
                tvec=[tvec;t];
                mineigvec=[mineigvec;min(eig(Y1Y1_sum))];
             end
        end
        prevcheck=t;
      end
